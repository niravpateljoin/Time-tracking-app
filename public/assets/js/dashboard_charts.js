    ...
    var _MonthlySalesAreaChart = function(element, height, color) {
        ...
        if($(element).length > 0) {
            ...
            d3.json('/assets/global_assets/demo_data/dashboard/monthly_sales.json', function (error, data) {
                ...
                data = monthlyChartRequestedData;
                console.log(data);
                ...
                function mousemove() {
                    ...
                    if(mousex >= (d3Container.node().getBoundingClientRect().width - focusText.select('text').node().getBoundingClientRect().width - margin.right - margin.left)) {
                        focusText.select('text').attr('text-anchor', 'end').attr('x', function () { return (x(d.date) - 15) + 'px' }).text(formatDate(d.date) + ' - ' + d.value + ' Payment Request');
                    }
                    else {
                        focusText.select('text').attr('text-anchor', 'start').attr('x', function () { return (x(d.date) + 15) + 'px' }).text(formatDate(d.date) + ' - ' + d.value + ' Payment Request');
                    }
                }
